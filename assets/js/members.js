document.addEventListener("DOMContentLoaded", () => {
  globalThis.loadJsonGrid("/assets/json/members.json", "members-grid", (member) => `
    <div class="glass card-hover rounded-2xl p-6 flex flex-col h-full">
      ...
      <p class="text-slate-400 text-sm mb-6 flex-grow">${member.bio}</p>
      <div class="flex items-center justify-between text-xs text-slate-400 border-t border-slate-700/50 pt-4 mt-auto">
        <div class="flex items-center space-x-2">
          <img src="/assets/izom.jpeg" class="w-3 h-3">
          <span>Izom.Net Team</span>
        </div>
        <a href="${member.github}" target="_blank" rel="noopener noreferrer" class="relative z-10 inline-flex items-center justify-center p-1 text-slate-400 hover:text-white transition-colors">
